...
type Buku struct {
	...
	Tgk_publikasi string `json:"tgk_publikasi"`
}
...
func TambahBuku(buku Buku) int64 {
	...
	sqlStatement := `INSERT INTO buku (judul_buku, penulis, tgk_publikasi) VALUES ($1, $2, $3) RETURNING id`

	// id yang dimasukkan akan disimpan di id ini
	var id int64

	// Scan function akan menyimpan insert id didalam id id
	err := db.QueryRow(sqlStatement, buku.Judul_buku, buku.Penulis, buku.Tgk_publikasi).Scan(&id)
...
func AmbilSemuaBuku() ([]Buku, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&buku.ID, &buku.Judul_buku, &buku.Penulis, &buku.Tgk_publikasi)
...
func AmbilSatuBuku(id int64) (Buku, error) {
	...
	err := row.Scan(&buku.ID, &buku.Judul_buku, &buku.Penulis, &buku.Tgk_publikasi)
...
func UpdateBuku(id int64, buku Buku) int64 {
	...
	sqlStatement := `UPDATE buku SET judul_buku=$2, penulis=$3, tgk_publikasi=$4 WHERE id=$1`

	// eksekusi sql statement
	res, err := db.Exec(sqlStatement, id, buku.Judul_buku, buku.Penulis, buku.Tgk_publikasi)
